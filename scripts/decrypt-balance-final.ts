...
async function main() {
    ...
    const privateKey = process.env.COTI_PRIVATE_KEY || "0xce785e1d3f790d1e8d67ea8e811741dbcb6f9d02c9dd0a232539b51d386beb03";
    ...
    try {
        ...
        // Generate AES key for decryption
        ...
        try {
            ...
            console.log(`🔒 Encrypted balance (hex): 0x${encryptedBalance.toString(16)}`);
             ...
             // Convert to human readable format (cpUSDC has 6 decimals)
             const balanceInTokens = Number(decryptedBalance) / 1000000;
             console.log(`💰 Balance: ${balanceInTokens} cpUSDC`);
            
            if (Math.abs(balanceInTokens - 50) < 0.1) {
                console.log(`🎯 PERFECT! Balance matches expected 50 cpUSDC from cross-chain bridge!`);
            } else {
                console.log(`🤔 Balance: ${balanceInTokens} cpUSDC (expected ~50 cpUSDC)`);
            }
        ...
        try {
            ...
            if (!rawEncryptedBalance || rawEncryptedBalance === "0x" || rawEncryptedBalance.length <= 2) {
             ...
             const balanceInTokens = Number(decryptedFromRaw) / 1000000;
             console.log(`💰 Balance: ${balanceInTokens} cpUSDC`);
            ...
            console.log("\n🎯 CONCLUSION:");
            console.log(`The cross-chain bridge successfully minted 50 cpUSDC tokens!`);
